...
from elements.element_F.score_with_API_F import score_documents_with_api
...
async def check_saved_results():
    ...
    print("USING F LOCAL CHECK_SAVED_RESULTS")

    if last_metrics is None:
        return {"status": "NO RESULTS", "message": "Run scoring first."}

    if last_mode == "legacy":
        baseline_file = Path("config/element_F/baseline_metrics_legacy.json")
    else:
        baseline_file = Path("config/element_F/baseline_metrics_current.json")
...
def compute_gpt_metrics(df: pd.DataFrame) -> dict:
    ...
    raw_subs = [f"F{i}" for i in range(1, 2)]
    ...
    final_subs = [f"F{i}_final" for i in range(1, 2)]
...
def root():
    ...
    return HTMLResponse(html.replace("__ELEMENT__", "F"))
...
def apply_calibration_pipeline(df, mode):
    ...
    df["element_score_raw"] = df[[f"F{k}" for k in range(1, 2)]].mean(axis=1)
    ...
    for k in range(1, 2):
        df[f"F{k}_final"] = df[f"F{k}"]

    for idx, row in df.iterrows():
        rec = reconcile_integer_subscores(
            row=row.to_dict(),
            keys=[f"F{k}" for k in range(1, 2)],
            target_element_col="element_score_target"
        )
        for k, v in rec.items():
            df.loc[idx, f"{k}_final"] = v

    df["element_score_final"] = df[[f"F{k}_final" for k in range(1, 2)]].mean(axis=1)
...
def process_files_background(job_id: str, file_payloads, mode: str):
    ...
    for i, file_data in enumerate(file_payloads):
        ...
        if filename.lower().endswith(".csv"):
            ...
            blended = "v1.2" if mode == "legacy" else "v1.62"
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("scorer_app_F:app", host="127.0.0.1", port=8000, reload=True)