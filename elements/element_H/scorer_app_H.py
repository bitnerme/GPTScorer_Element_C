...
from elements.element_H.score_with_API_H import score_documents_with_api
...
CURRENT_A =  0.87 #1.0 2026
CURRENT_B =  0.0  #0.37 2026
...
async def check_saved_results():
    ...
    if last_mode == "legacy":
        baseline_file = Path("config/element_H/baseline_metrics_legacy.json")
    else:
        baseline_file = Path("config/element_H/baseline_metrics_current.json")
...
def compute_gpt_metrics(df: pd.DataFrame) -> dict:
    ...
    raw_subs = [f"H{i}" for i in range(1, 6)]
    ...
    final_subs = [f"H{i}_final" for i in range(1, 6)]
...
def root():
    ...
    return HTMLResponse(html.replace("__ELEMENT__", "H"))
...
def apply_calibration_pipeline(
    ...
    df["element_score_raw"] = df[[f"H{k}" for k in range(1, 6)]].mean(axis=1)
    ...
    for k in range(1, 6):
        df[f"H{k}_final"] = df[f"H{k}"]

    for idx, row in df.iterrows():
        rec = reconcile_integer_subscores(
            row=row.to_dict(),
            keys=[f"H{k}" for k in range(1, 6)],
            target_element_col="element_score_target"
        )
        for k, v in rec.items():
            df.loc[idx, f"{k}_final"] = v

    df["element_score_final"] = df[[f"H{k}_final" for k in range(1, 6)]].mean(axis=1)
...
def process_files_background(job_id: str, file_payloads, mode: str):
    ...
    for i, file_data in enumerate(file_payloads):
        ...
        if filename.lower().endswith(".csv"):
            ...
            blended = "v1.0" if mode == "legacy" else "v2.0"
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("scorer_app_H:app", host="127.0.0.1", port=8000, reload=True)