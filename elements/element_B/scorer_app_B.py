...
from elements.element_B.score_with_API_B import score_documents_with_api
...
async def check_saved_results():
    ...
    if last_mode == "legacy":
        baseline_file = Path("config/element_B/baseline_metrics_legacy.json")
    else:
        baseline_file = Path("config/element_B/baseline_metrics_current.json")
...
print("### RUNNING THIS scorer_app_B.py ###")
print(__file__)
...
ELEMENT_NAME = "element_B"   # Change per element
ELEMENT_CODE = "B"           # Change per element
...
def compute_gpt_metrics(df: pd.DataFrame) -> dict:
    ...
    raw_subs = [f"B{i}" for i in range(1, 3)]
...
def apply_calibration_pipeline(df, mode):
    ...
    df["element_score_final"] = df[[f"B{k}_final" for k in range(1,3)]].mean(axis=1)
...
def process_files_background(job_id: str, file_payloads, mode: str):
    ...
    for i, file_data in enumerate(file_payloads):
        ...
        if filename.lower().endswith(".csv"):
            ...
            blended = get_blended_model("B", mode)
    ...
    final_cols = [f"B{i}_final" for i in range(1,3)]
    if all(c in df.columns for c in final_cols):
        print("MEAN(B*_final):", df[final_cols].mean(axis=1).mean())

    raw_cols = [f"B{i}" for i in range(1,3)]
    if all(c in df.columns for c in raw_cols):
        print("MEAN(B*):", df[raw_cols].mean(axis=1).mean())
    ...
    # ============================================================
    flag_cols = [f"B{k}_flag" for k in range(1, 3) if f"B{k}_flag" in df.columns]
    rat_cols = [f"B{k}_rationale" for k in range(1, 3) if f"B{k}_rationale" in df.columns]
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("scorer_app_B:app", host="127.0.0.1", port=8000, reload=True)
