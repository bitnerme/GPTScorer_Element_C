...
async def check_saved_results():
    ...
    if last_mode == "legacy":
        baseline_file = Path("config/element_A/baseline_metrics_legacy.json")
    else:
        baseline_file = Path("config/element_A/baseline_metrics_current.json")
...
print("### RUNNING THIS scorer_app_A.py ###")
print(__file__)
...
ELEMENT_NAME = "element_A"   # Change per element
ELEMENT_CODE = "A"           # Change per element
...
def compute_gpt_metrics(df: pd.DataFrame) -> dict:
    ...
    # Prefer element_score_raw if present; otherwise compute from raw subscores A1..A6
    raw_subs = [f"A{i}" for i in range(1, 7)]
    for c in raw_subs:
        if c in df.columns:
    ...
    # Prefer A*_final if present; else fall back to raw C*
    final_subs = [f"A{i}_final" for i in range(1, 7)]
    if all(c in df.columns for c in final_subs):
        for c in final_subs:
            df[c] = pd.to_numeric(df[c], errors="coerce")
        df["element_score_final"] = df[final_subs].mean(axis=1)
        final_source = "A*_final"
    else:
        df["element_score_final"] = df[raw_subs].mean(axis=1)
        final_source = "A* (fallback)"
...
def process_files_background(job_id: str, file_payloads, mode: str):
    ...
    for i, file_data in enumerate(file_payloads):
        ...
        if filename.lower().endswith(".csv"):
            ...
            blended = "v1.0" if mode == "legacy" else "v1.2"
    ...
    # ============================================================
    flag_cols = [f"A{k}_flag" for k in range(1, 7) if f"A{k}_flag" in df.columns]
    rat_cols = [f"A{k}_rationale" for k in range(1, 7) if f"A{k}_rationale" in df.columns]
    ...
    #    with open("config/element_A/baseline_metrics_legacy.json", "w") as f:
    #        json.dump(last_metrics, f, indent=2)
    #elif last_metrics is not None and last_mode == "current":
    #    with open("config/element_A/baseline_metrics_current.json", "w") as f:
    #        json.dump(last_metrics, f, indent=2)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("scorer_app_A:app", host="127.0.0.1", port=8000, reload=True)
