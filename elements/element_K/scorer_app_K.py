...
from elements.element_K.score_with_API_K import score_documents_with_api
...
async def check_saved_results():
    ...
    if last_mode == "legacy":
        baseline_file = Path("config/element_K/baseline_metrics_legacy.json")
    else:
        baseline_file = Path("config/element_K/baseline_metrics_current.json")
...
def compute_gpt_metrics(df: pd.DataFrame) -> dict:
    ...
    raw_subs = [f"K{i}" for i in range(1, 4)]
    ...
    final_subs = [f"K{i}_final" for i in range(1, 4)]
...
def apply_calibration_pipeline(df, mode):
    ...
    for k in range(1, 4):
        df[f"K{k}_final"] = df[f"K{k}"]

    for idx, row in df.iterrows():
        rec = reconcile_integer_subscores(
            row=row.to_dict(),
            keys=[f"K{k}" for k in range(1, 4)],
    ...
    df["element_score_final"] = df[[f"K{k}_final" for k in range(1, 4)]].mean(axis=1)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("scorer_app_k:app", host="127.0.0.1", port=8000, reload=True)