...
from elements.element_J.score_with_API_J import score_documents_with_api
...
async def check_saved_results():
    ...
    if last_mode == "legacy":
        baseline_file = Path("config/element_J/baseline_metrics_legacy.json")
    else:
        baseline_file = Path("config/element_J/baseline_metrics_current.json")
...
def compute_gpt_metrics(df: pd.DataFrame) -> dict:
    ...
    raw_subs = [f"J{i}" for i in range(1, 4)]
    ...
    final_subs = [f"J{i}_final" for i in range(1, 4)]
...
def root():
    ...
    return HTMLResponse(html.replace("__ELEMENT__", "J"))
...
def apply_calibration_pipeline(
    ...
    for k in range(1, 4):
        df[f"J{k}_final"] = df[f"J{k}"]

    for idx, row in df.iterrows():
        rec = reconcile_integer_subscores(
            row=row.to_dict(),
            keys=[f"J{k}" for k in range(1, 4)],
    ...
    df["element_score_final"] = df[[f"J{k}_final" for k in range(1, 4)]].mean(axis=1)
...
def process_files_background(job_id: str, file_payloads, mode: str):
    ...
    for i, file_data in enumerate(file_payloads):
        ...
        if filename.lower().endswith(".csv"):
            ...
            blended = "v1.0" if mode == "legacy" else "v1.5"
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("scorer_app_J:app", host="127.0.0.1", port=8000, reload=True)