...
from elements.element_I.score_with_API_I import score_documents_with_api
...
async def check_saved_results():
    ...
    if last_mode == "legacy":
        baseline_file = Path("config/element_I/baseline_metrics_legacy.json")
    else:
        baseline_file = Path("config/element_I/baseline_metrics_current.json")
...
def compute_gpt_metrics(df: pd.DataFrame) -> dict:
    ...
    raw_subs = [f"I{i}" for i in range(1, 5)]
    ...
    final_subs = [f"I{i}_final" for i in range(1, 5)]
...
def root():
    ...
    return HTMLResponse(html.replace("__ELEMENT__", "I"))
...
def apply_calibration_pipeline(df, mode):
    ...
    df["element_score_raw"] = df[[f"I{k}" for k in range(1, 5)]].mean(axis=1)
    ...
    print("I CALIBRATION MODE:", mode, "A:", a, "B:", b)


    df["element_score_target"] = (a * df["element_score_raw"] + b).clip(0, 5)

    print(df[["element_score_raw", "element_score_target"]].head(10))

    for k in range(1, 5):
        df[f"I{k}_final"] = df[f"I{k}"]

    for idx, row in df.iterrows():
        rec = reconcile_integer_subscores(
            row=row.to_dict(),
            keys=[f"I{k}" for k in range(1, 5)],
    ...
    df["element_score_final"] = df[[f"I{k}_final" for k in range(1, 5)]].mean(axis=1)
...
def process_files_background(job_id: str, file_payloads, mode: str):
    ...
    for i, file_data in enumerate(file_payloads):
        ...
        if filename.lower().endswith(".csv"):
            ...
            blended = "v1.1" if mode == "legacy" else "v1.2"
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("scorer_app_I:app", host="127.0.0.1", port=8000, reload=True)