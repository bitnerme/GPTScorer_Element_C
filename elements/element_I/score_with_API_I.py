...
SYSTEM_PROMPT = """You are a rigorous engineering design evaluator applying the Element I rubric consistently and professionally.
...
- Explain strengths or weaknesses in whether important design requirements were actually tested (I1).
- Discuss the clarity and completeness of the testing procedure, including how data was gathered and analyzed (I2).
- Evaluate the detail of the design analysis, including use of raw data, charts, graphs, photos, tables, or other visual evidence (I3).
- Discuss how well the student summarized the implications of the data for design success, design changes, next steps, or solving the problem (I4).
- Acknowledge when requirements were inferred from testing and analysis rather than explicitly stated.
- Provide 2–4 specific, actionable recommendations for improving testing documentation, data quality, analysis detail, visual evidence, or implications.
...
The narrative should focus on completed testing, collected data, analysis of results, and implications for the design.
...
RUBRIC_PATH = Path(__file__).resolve().parent / "MyDesign_Element_I_Scoring_Rubric.txt"
...
def score_document(filename, content, blended_model):
    ...
    if blended_model == "v1.1":
        ...
        You are scoring a MyDesign Element I engineering document using the Element I blended scoring model v1.1.
        ...
        Element I evaluates completed testing, data collection, analysis, and implications for the design.

        FIRST:
        Infer the major design requirements from the testing and analysis content when they are not explicitly listed.

        SECOND:
        Identify what tests were actually conducted, what data was collected, how results were analyzed, and what conclusions or implications were drawn.

        Score conservatively when:
        - the document describes a plan rather than completed testing
        - requirements are mentioned but not actually tested
        - procedures are vague
        - data is missing or unclear
        - analysis is mostly opinion rather than evidence-based
        - implications are generic or disconnected from results

        Allow partial credit when:
        - test procedures are implied but logically understandable
        - data or outcomes are summarized even if raw data is incomplete
        - narrative analysis clearly interprets trends or design success
        - reflection shows meaningful understanding of test results

        Do not award points for:
        - headings alone
        - blank templates
        - vague claims that testing was done
        - unsupported conclusions
        - copied planning language from Element H without evidence of execution

        If the document contains only vague statements, incomplete testing descriptions, generic reflections, unsupported conclusions, or minimal evidence of completed testing and analysis, scores of 0 may be appropriate for one or more subelements.
        ...
        "I1": {{"score": X, "rationale": "..." }},
        "I2": {{"score": X, "rationale": "..." }},
        "I3": {{"score": X, "rationale": "..." }},
        "I4": {{"score": X, "rationale": "..." }},
        "narrative_feedback": ""
        }}
        """
        ...
        You are scoring a MyDesign Element I engineering document using the Element I current scoring model v1.2.

        Apply the same evidence standards as the Legacy v1.1 model, but assume cleaner document structure and less OCR damage.
        ...
        Score based on evidence of completed testing and analysis, not on formatting or template completion alone.

        Allow credit for:
        - implied but coherent testing procedures
        - summarized data when raw data is incomplete
        - clear interpretation of results
        - meaningful discussion of design implications
        - next steps connected to test outcomes

        Score conservatively when:
        - the document mainly describes planned testing
        - results are vague or unsupported
        - analysis lacks data connection
        - implications are generic
        - requirements are mentioned but not tested
   ...
   # === Continue with parsing logic ===
    try:
        ...
        for i in range(1,5):
            response_dict[f"I{i}"] = int(response_dict.get(f"I{i}",0))
        ...
        for i in range(1,5):
            response_dict[f"I{i}_api"] = int(response_dict[f"I{i}"])

        scores = [response_dict[f"I{i}_api"] for i in range(1,5)]
        response_dict["element_score_api"] = sum(scores) / len(scores)

        # --- Validate expected fields (FAIL LOUDLY) ---
        for i in range(1, 5):
            assert f"I{i}" in response_dict, f"Missing I{i}"
            assert f"I{i}_rationale" in response_dict, f"Missing I{i}_rationale"
...
def main(folder_path, output_path, blended_version):
    ...
    for idx, filename in enumerate(all_files, start=1):
        ...
        try:
            ...
            row["incomplete_response"] = any(
                k not in response_dict for k in [f"I{i}" for i in range(1, 5)]
            )

            # Ensure h scores are integers
            for i in range(1, 5):
                row[f"I{i}"] = int(row.get(f"I{i}", 0))

            print(
                filename,
                [row[f"I{i}_api"] for i in range(1, 5)],
    ...
    scores = [f"I{i}" for i in range(1, 5)]
    api_scores = [f"I{i}_api" for i in range(1, 5)]  # 👈 NEW

    flags = [f"I{i}_flag" for i in range(1, 5)]
    rationales = [f"I{i}_rationale" for i in range(1, 5)]
...
def score_documents_with_api(documents, blended_version):
    ...
    for idx, doc in enumerate(documents, start=1):
        ...
        for i in range(1, 5):
            ...
            print("I key value:", i, type(response_dict.get(f"I{i}")), response_dict.get(f"I{i}"))
            row[f"I{i}"] = int(response_dict.get(f"I{i}", 0))
            row[f"I{i}_rationale"] = response_dict.get(f"I{i}_rationale", "")
        ...
        for i in range(1, 5):
            row[f"I{i}_api"] = int(
                response_dict.get(f"I{i}_api", response_dict.get(f"I{i}", 0))
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--blended-model", choices=["v1.1", "v1.2"], default="v1.2",
